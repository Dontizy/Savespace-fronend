...
const schema = z.object({
    ...
    email: z.email(),
    password: z.string().min(8, "Password must not be empty, minimum of 8 characters")
})

type signupData = z.infer<typeof schema>

const Signup = () => {
    const { register, handleSubmit, formState: { errors } } = useForm<signupData>({resolver:zodResolver(schema)})

    const signupHandler: SubmitHandler<signupData> = (data) => {
        console.log(data);
        ...
        <>
            <main className="h-screen flex justify-center items-center">
                <div className="flex items-center justify-center flex-col p-4 rounded-2x bg-white shadow-2xl">
                    ...
                    <form className="htmlForm-login " onSubmit={handleSubmit(signupHandler)}>
                        <div className="flex flex-col gap-2 mt-2 md:mt-4">
                            ...
                            <input id="firstName" {...register('firstName')} type="text" name="firstName" placeholder="First Name"
                                ...
                                {errors.firstName && <p className="text-sm p-1 text-red-800 p-0 m-0">{errors.firstName?.message}</p>}
                        </div>
                        ...
                        <div className="flex flex-col gap-2 mt-2 md:mt-4">
                            ...
                            <input id="lastName" {...register('lastName')} type="text" name="lastName" placeholder="Last Name"
                                className="block w-auto md:min-w-100 outline outline-gray-400  grow bg-transparent rounded-md py-1 md:py-1.5 pr-5 pl-1 text-base  placeholder:text-gray-500 focus:outline-offset-2 focus:outline-blue-500  sm:text-sm/6" />
                            {errors.lastName && <p className="text-sm text-red-800 p-0 m-0">{errors.lastName?.message}</p>}
                        </div>
                        ...
                        <div className="flex flex-col gap-1 mt-2 md:mt-4">
                            ...
                            <div>
                                <input id="email" type="email" {...register('email')} name="email" placeholder="email"
                                    className="block w-auto md:min-w-100 outline outline-gray-400  grow bg-transparent rounded-md py-1 md:py-1.5 pr-5 pl-1 text-base  placeholder:text-gray-500 focus:outline-offset-2 focus:outline-blue-500  sm:text-sm/6" />
                                {errors.email && <p className="text-sm text-red-800 py-1 m-0">{errors.email?.message}</p>}
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2 mt-2 md:mt-4">
                            ...
                            <input id="password" type="password" {...register('password')} name="password" placeholder="password"
                                className="block w-auto md:min-w-100 outline outline-gray-400  grow bg-transparent rounded-md py-1 md:py-1.5 pr-5 pl-1 text-base  placeholder:text-gray-500 focus:outline-offset-2 focus:outline-blue-500  sm:text-sm/6" />
                                {errors.password && <p className="text-sm text-red-800 p-0 m-0">{errors.password?.message}</p>}
                        </div>
                        <div>
                            <button type="submit"
